...
const labels={"trial":"Пробный — 24 часа","14d":"14 дней","1m":"1 месяц","2m":"2 месяца","6m":"6 месяцев","12m":"12 месяцев"};const referralBonus={"1m":14,"6m":30,"12m":60};
...
function addPeriod(start,p){const d=parse(start);if(p==="trial"){d.setDate(d.getDate()+1);return iso(d)}if(p==="14d"){d.setDate(d.getDate()+14);return iso(d)}const monthsMap={"1m":1,"2m":2,"6m":6,"12m":12};if(!(p in monthsMap))throw new Error("Неизвестный тариф: "+p);const mm=monthsMap[p],day=d.getDate();d.setDate(1);d.setMonth(d.getMonth()+mm);const last=new Date(d.getFullYear(),d.getMonth()+1,0).getDate();d.setDate(Math.min(day,last));return iso(d)}
...
const options=()=>`<option value="trial">Пробный — 24 часа</option><option value="14d">14 дней</option><option value="1m">1 месяц</option><option value="2m">2 месяца</option><option value="6m">6 месяцев</option><option value="12m">12 месяцев</option>`;
function addDaysToDate(startISO,n){const d=parse(startISO);d.setDate(d.getDate()+n);return iso(d)}
function fillReferrers(){
  const sel=$("referrer");
  if(!sel)return;
  const current=sel.value;
  sel.innerHTML='<option value="">Никто / не указан</option>'+clients
...
function updateReferralHint(){
  const bonus=referralBonus[$("period").value]||0;
  const who=$("referrer").value;
...
async function load(){const{data,error}=await sb.from("clients").select("*").order("end_date",{ascending:true});if(error)return msg(error.message,true);clients=data||[];fillReferrers();render()}
...
$("addForm").addEventListener("submit",async e=>{
  ...
  if(referrerId&&bonus>0){
    ...
    if(ref){
      ...
      const newEnd=addDaysToDate(base,bonus);
      const{error:bonusError}=await sb.from("clients").update({end_date:newEnd}).eq("id",referrerId);
      if(bonusError){
        msg("Клиент добавлен, но бонус пригласившему не начислился: "+bonusError.message,true);
        await load();
        return;
      }
    }
  ...
  preview();
  updateReferralHint();
  msg(bonus&&referrerId?`Клиент добавлен. Пригласившему +${bonus} дней.`:"Клиент добавлен");
  load();
});
...
if(b.dataset.a==="edit"){$("editId").value=c.id;$("editName").value=c.name||"";$("editPhone").value=c.phone||"";$("editStart").value=c.start_date;$("editDaysLeft").value=Math.max(0,days(c.end_date));$("editNote").value=c.note||"";$("editDialog").showModal();return}
...
$("editForm").addEventListener("submit",async e=>{e.preventDefault();const id=Number($("editId").value),left=Math.max(0,parseInt($("editDaysLeft").value||"0",10)),end=parse(today());end.setDate(end.getDate()+left);const{error}=await sb.from("clients").update({name:$("editName").value.trim(),phone:$("editPhone").value.trim(),start_date:$("editStart").value,end_date:iso(end),note:$("editNote").value.trim()}).eq("id",id);if(error)return msg(error.message,true);$("editDialog").close();msg("Изменения сохранены");load()});
...
sb.auth.onAuthStateChange(async(_e,s)=>{session=s;const yes=!!s;$("authScreen").classList.toggle("hidden",yes);$("app").classList.toggle("hidden",!yes);if(yes){$("startDate").value=today();preview();await load();updateReferralHint()}});
(async()=>{const{data}=await sb.auth.getSession();session=data.session;const yes=!!session;$("authScreen").classList.toggle("hidden",yes);$("app").classList.toggle("hidden",!yes);if(yes){$("startDate").value=today();preview();await load();updateReferralHint()}})();
})();